...
export function curveHorizon({ start, control, end }: LinePositions, padding = 0) {
  ...
  return `M ${start.x} ${start.y}` + `L ${control.x} ${control.y}` + `Q ${controlX} ${end.y} ${end.x - padding} ${end.y}`
}

export function curveVertical({ start, control, end }: LinePositions, padding = 0) {
  return `M ${start.x} ${start.y}` + `L ${control.x} ${control.y}` + `Q ${end.x} ${control.y} ${end.x} ${end.y - padding}`
}

export function sinusHorizon({ start, control, end }: LinePositions) {
  const dx = end.x - control.x
  const flexX = (end.x + control.x) / 2
  return `M ${start.x} ${start.y}` + `L ${control.x} ${control.y}` + `C ${end.x - dx / 4} ${control.y} ${flexX} ${end.y} ${end.x} ${end.y}`
}

export function sinusVertical({ start, control, end }: LinePositions) {
  const dy = end.y - control.y
  const flexY = (end.y + control.y) / 2
  return `M ${start.x} ${start.y}` + `L ${control.x} ${control.y}` + `C ${control.x} ${end.y - dy / 4} ${end.x} ${flexY} ${end.x} ${end.y}`
}
...
export function elbowVertical({ start, control, end }: LinePositions) {
  return `M ${start.x} ${start.y}` + `L ${control.x} ${control.y}` + `L ${end.x} ${control.y}` + `L ${end.x} ${end.y}`
}

export function elbowHorizon({ start, control, end }: LinePositions) {
  return `M ${start.x} ${start.y}` + `L ${control.x} ${control.y}` + `L ${control.x} ${end.y}` + `L ${end.x} ${end.y}`
}
...
export function skewElbowVertical({ start, control, end }: LinePositions) {
  ...
  return `M ${start.x} ${start.y}` + `L ${control.x} ${control.y}` + `L ${flexPos.x} ${flexPos.y}` + `L ${end.x} ${end.y}`
}
