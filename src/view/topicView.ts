...
export default class TopicView extends View {
  ...
  private _initSVGStructure() {
    ...
    this._topicShapeFill = topicShapeGroup
      .path()
      .data("name", "topic-shape-fill");

    this._topicContent = topicShapeGroup.group().data("name", "topic-content");
    this._topicInnerElementGroup = this._topicContent
      .group()
      .data("name", "inner-element-group");
  }
  ...
  set topicShapeGroupPosition(topicShapeGroupPosition: Position) {
    const newPositionDirty =
      !this._topicShapeGroupPosition ||
      !isSamePosition(this._topicShapeGroupPosition, topicShapeGroupPosition);
    if (newPositionDirty) {
  ...
  set topicContentPosition(topicContentPosition: Position) {
    const newPositionDirty =
      !this._topicContentPosition ||
      !isSamePosition(this._topicContentPosition, topicContentPosition);
    if (newPositionDirty) {
  ...
  set topicInnerElementPosition(topicInnerElementPosition: Position) {
    const newPositionDirty =
      !this._topicInnerElementPosition ||
      !isSamePosition(
        this._topicInnerElementPosition,
  ...
  render(parentView: View) {
    ...
    if (this.topicContentPositionDirty) {
      ...
      this._topicContent.translate(
        this.topicContentPosition.x,
        this.topicContentPosition.y
      );
